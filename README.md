...
Complete Flow: From Input to SMS Alert
1. User Accesses the Web App
- URL: http://localhost:5000
- The index.html page is rendered via Flask route `/`.
2. User Enters Network Traffic Features
- The user fills in 12 numerical inputs in a form (e.g., `rate`, `sttl`, `sload`, etc.).
- These values are submitted via POST request to the `/predict` route.
Example HTML form snippet:
<form action="/predict" method="POST">
  <input name="rate" value="0.1">
  ...
  <button type="submit">Predict</button>
</form>
3. Flask Receives and Processes Input
- `app.py` receives the POST request at the `/predict` route:
```python
input_features = [float(request.form[feature]) for feature in FEATURE_NAMES]
input_df = pd.DataFrame([input_features], columns=FEATURE_NAMES)
```
- The user inputs are converted into a Pandas DataFrame with correct feature order.
4. Model Prediction
- A pre-trained logistic regression model (`logistic_regressor_binary.pkl`) is used:
```python
prediction = model.predict(input_df)[0]
```
- The result is a binary class:
  - `0` = Normal
  - `1` = Abnormal / Attack
5. Send SMS If Abnormal Traffic Is Detected
If prediction is `1`, Flask uses **Twilio** to send an SMS alert:
```python
...
if predicted_class == 1:
    ...
    message = client.messages.create(
        body="⚠️ Alert: Abnormal network traffic detected! Please investigate.",
        from_=TWILIO_PHONE_NUMBER,
        to=TO_PHONE_NUMBER
    )
```
The message gets sent instantly to the specified mobile number.
6. Return Result to Frontend
- The prediction is passed to `index.html` using:
```python
return render_template("index.html", prediction=f"Predicted Class: {predicted_class}")
```
- The frontend uses conditionals to display the result:
  - Green message for `Normal`
  - Red warning and explanation for `Abnormal`
Summary of Technologies Used
Component	Technology Used
Backend Server	Flask
ML Model	Logistic Regression (pickle)
Input Handling	HTML Forms
Output Display	Jinja2 (Flask templates)
SMS Alert	Twilio Python SDK
